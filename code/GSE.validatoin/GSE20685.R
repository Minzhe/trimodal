...
load("data/GSE/UNC_GSE20685_clean.RData")
...
# 
# pdf("~/project/ORMDL3/metabric/trimodal/GSE20685.pdf", width = 6, height = 8)
# par(mfrow = c(3, 2))
# 
# for (hit in hits_normal)
# {
#       # get expression data
#       if (!hit %in% colnames(expr)) {
#             next
#       }
#       exp_hit = expr[hit, ]
#       
...
#       trimodal = list(
#             TI = -1,
#             cutoff12 = quantile(exp_hit, TI_metabric[hit, "pi1"], na.rm = T),
#             cutoff23 = quantile(exp_hit, 1 - TI_metabric[hit, "pi3"], na.rm = T)
#       )
#       
#       # plot survival data
#       results[hit, "TI"] = trimodal$TI
#       cat_exp = categorize(exp_hit, trimodal$cutoff12, trimodal$cutoff23)
#       results[hit, c("z_12", "p_12", "z_23", "p_23")] = plot_surv(clin, cat_exp, "GSE20685", other =
#                                                                         NULL)
# }
# 
# dev.off()
...
# plot survival data
cat_exp <- categorize(exp_hit, trimodal$cutoff12, trimodal$cutoff23)
plot_surv(clin, cat_exp, "GSE20685", print = T, plot = T)
