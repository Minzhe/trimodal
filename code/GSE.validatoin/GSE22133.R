...
load("data/GSE/UNC_GSE22133_clean.RData")
colnames(clin)[colnames(clin) == "OS"] = "T"
colnames(clin)[colnames(clin) == "Death"] = "last_follow_up_status"
clin$last_follow_up_status[clin$T > 20] = 0
clin$T[clin$T > 20] = 20
...
pdf("~/project/ORMDL3/metabric/trimodal/GSE22133.pdf", width = 6, height = 8)
par(mfrow = c(3, 2))
...
for (hit in hits_normal) {
      ...
      results[hit, c("z_12", "p_12", "z_23", "p_23")] <- plot_surv(clin, cat_exp, "GSE22133", other = NULL)

      # grade and ER grouping
      results[hit, c("grade_12", "grade_23", "odd_12", "odd_23")] <- check_anno(cat_exp, clin$Grade, clin$Subtype == "Basal")
}
...
# plot survival data
cat_exp <- categorize(exp_hit, trimodal$cutoff12, trimodal$cutoff23)
plot_surv(clin, cat_exp, "GSE22133", print = T, plot = T)
