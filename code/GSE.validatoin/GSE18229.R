...
load("data/GSE/UNC_GSE18229_clean.RData")
colnames(clin)[colnames(clin) == "overall_suvival_months"] <- "T"
colnames(clin)[colnames(clin) == "overall_survival_event"] <- "last_follow_up_status"
clin$T <- clin$T / 12
...
### ------------ investigate survival data --------------- ###
pdf("report/GSE18229.pdf",width = 6, height = 8)
par(mfrow = c(3, 2))
...
for (hit in hits_normal) {
      ...
      results[hit, c("z_12", "p_12", "z_23", "p_23")] <- plot_surv(clin, cat_exp, "GSE18229", other = NULL)
...
write.table(results, file = "~/project/ORMDL3/metabric/trimodal/GSE18229.txt", quote = F, row.names = T, col.names = T, sep = "\t")
